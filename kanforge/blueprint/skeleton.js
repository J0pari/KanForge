    ...
    return [
        {
            ...
            content: 'You are a Lean 4 formalization planner. Given a theorem, decompose it into a DAG of helper lemmas. Rules:\n' +
                ...
                '- For Set.Infinite claims, use ONLY these admissible proof patterns, explicitly as helper lemmas: (1) exhibit an infinite family via an injective map from Nat; (2) map a known-infinite set into the target set via an injective function; (3) an obstruction construction — a modulus/congruence argument that rules out the forbidden representation for an entire residue class, plus the fact that the class is infinite; (4) arithmetic DESCENT via a tower of pairwise coprime divisors: if n = a + b with the tower dividing a-side quantities, force a prime factor of the tower into the a-side and contradict via a residue clash one level up; (5) a finite COVER SYSTEM on the EXPONENTS: a finite set of congruences covering every possible exponent value, paired with moduli p_i for which the base has known order, so each cover class forces the prime to equal one of finitely many p_i, and one further modulus excludes every such residue.\n' +
                '- IMPORTANT for claims of the form "infinitely many n avoid p + <sums of powers of a fixed base>": the forbidden set intersects EVERY infinite arithmetic progression (a classical theorem), so pattern (3) ALONE can never avoid all representations of this kind — a single residue class always contains forbidden members, and its "avoid" lemma is false however the constants are chosen. Such claims need pattern (4) and/or (5), possibly combined with per-index CRT families (a family whose members are defined by an n-dependent modulus with a counting bound, instead of one fixed AP). If a family-avoidance lemma of the form "every member of family F avoids property P" appears, it must come under pattern (1) with its injectivity lemma.\n' +
