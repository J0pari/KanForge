...
- **Intake gate (fail-closed)**: a mission may not enter the pipeline without a kernel-verified instance-probe ledger (`runs/<problem>/probes.json`, written by the single intake path `bench/validateFormalization.js --fc-file=... --emit-mission=<dir>`)
